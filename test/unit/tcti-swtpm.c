...
/*
 * This test exercises the timeout check of the receive function.
 */
static void
tcti_swtpm_receive_timeout_fail_test (void **state)
{
    TSS2_TCTI_CONTEXT *ctx = (TSS2_TCTI_CONTEXT*)*state;
    TSS2_TCTI_COMMON_CONTEXT *tcti_common = tcti_common_context_cast (ctx);
    TSS2_RC rc = TSS2_RC_SUCCESS;
...
tcti_swtpm_control_command_null_test (void **state)
{
    ...
    /* resp_sdu_len NULL with resp_sdu not NULL */
    rc = tcti_control_command (ctx, 0, NULL, 0, NULL, payload_out, NULL);
    assert_int_equal (rc, TSS2_TCTI_RC_BAD_REFERENCE);
}
/*
      ...
      char *argv[])
{
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown (tcti_swtpm_receive_eof_second_read_test,
                                         tcti_swtpm_setup,
                                         tcti_swtpm_teardown),
        cmocka_unit_test_setup_teardown (tcti_swtpm_receive_timeout_fail_test,
                                         tcti_swtpm_setup,
                                         tcti_swtpm_teardown),
