 ...
 *             (can be NULL if resp_sdu is NULL)
    ...
    uint32_t *resp_code, void *resp_sdu, size_t *resp_sdu_len)
{
    ...
    if (resp_sdu != NULL && resp_sdu_len == NULL) {
    ...
    if (resp_sdu_len != NULL) {
        *resp_sdu_len = resp_buf_len - sizeof(uint32_t);
        ...
        if (resp_sdu != NULL) {
            memcpy(resp_sdu, resp_buf +  sizeof(uint32_t), *resp_sdu_len);
        }
    }
...
TSS2_RC tcti_swtpm_reset(TSS2_TCTI_CONTEXT *tctiContext)
{
    TSS2_RC rc;
    ...
    const char *conf)
{
    ...
    rc = tcti_swtpm_set_locality(tctiContext, 3);
    if (rc != TSS2_RC_SUCCESS) {
